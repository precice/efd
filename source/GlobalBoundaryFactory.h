...
private:
  ...
  BoundaryStencil<FlowField>* _velocityStencils[6]; // ! A stencil for
                                                    // each face
  BoundaryStencil<FlowField>* _FGHStencils[6]; // ! A stencil for each
                                               // face
  BoundaryStencil<FlowField>* _moving[2]; // ! Pointers to the
                                          // moving wall stencils,
                                          // if any
  BoundaryStencil<FlowField>* _periodic[2]; // ! Pointers to the
                                            // periodic stencils, if
                                            // any
  BoundaryStencil<FlowField>* _outflow[2]; // ! Pointers for the
                                           // outflow conditions
  BoundaryStencil<FlowField>* _channelInput[2]; // ! For the velocity
                                                // input
  const Parameters& _parameters;                // ! Reference to the parameters

public:
  /** Constructor. Will initialize all the references to the boundaries and
   * create the
   * stencils.
   ...
   */
  GlobalBoundaryIterator<FlowField> getGlobalBoundaryVelocityIterator(
    FlowField& flowField);
