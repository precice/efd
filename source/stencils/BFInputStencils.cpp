...
FLOAT
computeVelocity3D(FlowField& flowField, int i, int j, int k, FLOAT stepSize,
  ...
  if (posY + 0.5 * dy >= stepSize) {
    // Get the size of the inlet in Y. A 3 is subtracted because of the boundary
    // cells
...
FLOAT
computeVelocity2D(FlowField& flowField, int i, int j, FLOAT stepSize,
                  const Parameters& parameters) {
...
BFInputVelocityStencil(const Parameters& parameters)
  : BoundaryStencil<FlowField>(parameters),
    // Here, the obstacle size is set to zero if it was set as negative at the
    // configuration
    _stepSize(parameters.bfStep.yRatio > 0.0 ? parameters.bfStep.yRatio *
              parameters.geometry.lengthY : 0.0) {
  if (_parameters.geometry.dim == 2) {
    ...
    for (int j = 0; j < _parameters.geometry.sizeY - 1; ++j) {
      ...
      if (posY + 0.5 * dy < _stepSize && _stepSize <= posY + dy + 0.5 *
          nextDy) {
        _stepSize = posY + dy;
        break;
      }
