...
/** this class computes the maximum value of max(velocity)/meshsize for all
 * grid cells. Originally, one would compute the max. velocity only and adapt
 * it with the meshsize afterwards. This, however, becomes inconsistent when
 * dealing with non-uniform, e.g. stretched, meshes, since the meshsize may be
 * different for every grid cell. We therefore determine the
 * max(velocity)/meshsize and synchronise this value over whole computational
 * domain.
 *  @author Philipp Neumann
 */
class MaxUStencil : public FieldStencil<FlowField>, public
                    BoundaryStencil<FlowField> {
private:
  FLOAT _maxValues[3]; // ! Stores the maximum module of every component

  /** Sets the maximum value arrays to the value of the cell if it surpasses the
   * current one.
   *
   ...
   */
  void
  cellMaxValue(FlowField& flowField, int i, int j);
   ...
   */
  void
  cellMaxValue(FlowField& flowField, int i, int j, int k);
...
public:
  ...
  /** Returns the array with the maximum modules of the components of the
   * velocity, divided by the respective local meshsize
   */
  const FLOAT*
  getMaxValues() const;
};
