...
apply(FlowField& flowField, int i, int j) {
  ...
  if ((obstacle & OBSTACLE_SELF) == 0) { // If this is a fluid cell
    if ((obstacle & OBSTACLE_RIGHT) == 0) { // Check whether the neighbor is
                                            // also fluid
      // we require a spatial finite difference expression for the pressure
      // gradient, evaluated
      // at the location of the u-component. We therefore compute the distance
      // of neighbouring
      // pressure values (dx) and use this as sort-of central difference
    ...
    if ((obstacle & OBSTACLE_LEFT) == 0) { // If the left neighbor is fluid
      ...
      // Here, we don't set the normal velocity to zero, since it has already
      // been taken care
...
apply(FlowField& flowField, int i, int j, int k) {
  ...
  if ((obstacle & OBSTACLE_SELF) == 0) {
    if ((obstacle & OBSTACLE_RIGHT) == 0) {
      const FLOAT dx = 0.5 * (_parameters.meshsize->getDx(i, j, k) +
    ...
    if ((obstacle & OBSTACLE_TOP) == 0) {
      const FLOAT dy = 0.5 * (_parameters.meshsize->getDy(i, j, k) +
    ...
    if ((obstacle & OBSTACLE_LEFT) == 0) { // If the left neighbour is a
                                           // fluid cell
