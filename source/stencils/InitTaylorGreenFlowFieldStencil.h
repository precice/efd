...
private:
  ...
  FLOAT*
  initializeDomainSize(const Parameters& parameters) const {
    FLOAT* domainSize = new FLOAT[3];

    if (domainSize == NULL) {
      handleError(1,
                  "InitTaylorGreenFlowFieldStencil: domainSize==NULL");
    }
  ...
  /** from the local grid coordinates i,j,k, computes the global coordinates of
   * the current cell and initializes
   *  the velocity field correspondingly
   */
  void
  computeGlobalCoordinates(FLOAT* coords, int i, int j, int k = 0) const {
    // TODO CHECK for uniform/stretched meshes
    coords[0] = _parameters.meshsize->getPosX(i, j, k) + 0.5 *
                _parameters.meshsize->getDx(i, j, k);
    coords[1] = _parameters.meshsize->getPosY(i, j, k) + 0.5 *
                _parameters.meshsize->getDy(i, j, k);

    if (_parameters.geometry.dim == 3) {
      coords[2] = _parameters.meshsize->getPosZ(i, j, k) + 0.5 *
                  _parameters.meshsize->getDz(i, j, k);
    }
    // Output for debugging
    // std::cout << "RankCorner " << _parameters.parallel.rank << ": " <<
    // _parameters.parallel.firstCorner[0] << "," <<
    // _parameters.parallel.firstCorner[1] << "," <<
    // _parameters.parallel.firstCorner[2] << std::endl;
    // std::cout << "Rank " << _parameters.parallel.rank << ": " << coords[0] <<
...
public:
  ...
  InitTaylorGreenFlowFieldStencil(
    ...
    : FieldStencil<FlowField>(parameters),
      _2pi(2.0 * 3.141592653589793238),
      _domainSize(initializeDomainSize(parameters))
  {}
  ...
  apply(FlowField& flowField, int i, int j) {
    ...
    velocity[0] = sin(_2pi * (coords[0] + 0.5 * _parameters.meshsize->getDx(i,
                                                                            j))
                      / _domainSize[0]) *
                  sin(_2pi * coords[1] / _domainSize[1]);
    velocity[1] = cos(_2pi * coords[0] / _domainSize[0]) *
                  cos(_2pi * (coords[1] + 0.5 * _parameters.meshsize->getDy(i,
                                                                            j))
                      / _domainSize[1]);
  }
  ...
  apply(FlowField& flowField, int i, int j, int k) {
    ...
    // initialize velocities
    velocity[0] = cos(_2pi * (coords[0] + 0.5 * _parameters.meshsize->getDx(i,
                                                                            j,
                                                                            k))
                      / _domainSize[0]) *
                  sin(_2pi * coords[1] / _domainSize[1]) *
    ...
    velocity[2] = sin(_2pi * coords[0] / _domainSize[0]) *
                  sin(_2pi * coords[1] / _domainSize[1]) *
                  cos(_2pi * (coords[2] + 0.5 * _parameters.meshsize->getDz(i,
                                                                            j,
                                                                            k))
                      / _domainSize[2]);
  }
