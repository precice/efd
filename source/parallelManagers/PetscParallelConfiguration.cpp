...
PetscParallelConfiguration::
locateNeighbors() {
  /*TODO: This function will have to be modified if the rank of a processor and
   * the subdomain can be associated by specification of the user
   */
  ...
  // If periodic boundaries declared, let the process itself deal with it,
...
createIndices() {
  ...
  _parameters.parallel.indices[0] = rank %
                                    _parameters.parallel.numProcessors[0];
  _parameters.parallel.indices[1] = (rank /
                                     _parameters.parallel.numProcessors[0]) %
                                    _parameters.parallel.numProcessors[1];
  _parameters.parallel.indices[2] = rank /
                                    (_parameters.parallel.numProcessors[0] *
                                     _parameters.parallel.numProcessors[1]);
}
...
int
PetscParallelConfiguration::
computeRankFromIndices(int i, int j, int k) const {
  ...
  if (_parameters.geometry.dim == 3) {
    nrank += k * _parameters.parallel.numProcessors[0] *
             _parameters.parallel.numProcessors[1];
  }
...
computeSizes() {
  ...
  for (int i = 0; i < dim; i++) {
    _parameters.parallel.sizes[i] =
      new PetscInt[_parameters.parallel.numProcessors[i]];
  }
  ...
  // Locate the position of the first element of the subdomain. Useful for
  // plotting later on.
  for (int i = 0; i < dim; i++) {
  ...
  // If the domain lies on an edge, add one to that direction, for the
  // artificial external pressures in the PETSc solver
  for (int i = 0; i < dim; i++) {
