...
// positions with the same iterator for both pressures and velocities.
    ...
    _velocity(VectorField(Nx + 3, Ny + 3)), _flags(IntScalarField(Nx + 3, Ny +
                                                                  3)),
...
FlowField(const Parameters& parameters)
  : _size_x(parameters.parallel.localSize[0]),
    ...
    _pressure(parameters.geometry.dim == 2 ? ScalarField(_size_x + 3, _size_y +
                                                         3) :
              ScalarField(_size_x + 3, _size_y + 3, _size_z + 3)),
    _velocity(parameters.geometry.dim == 2 ? VectorField(_size_x + 3, _size_y +
                                                         3) :
              VectorField(_size_x + 3, _size_y + 3, _size_z + 3)),
    _flags(parameters.geometry.dim == 2 ? IntScalarField(_size_x + 3, _size_y +
                                                         3) :
...
void
FlowField::
getPressureAndVelocity(FLOAT& pressure, FLOAT* const velocity, int i, int j, int
                       k) {
