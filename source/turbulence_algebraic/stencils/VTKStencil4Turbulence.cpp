#include "VTKStencil4Turbulence.h"


void VTKStencil4Turbulence::writeVTKHeader ( std::ostream & file) const {
...
void VTKStencil4Turbulence::writePoints (std::ostream & file) const {
...
VTKStencil4Turbulence::VTKStencil4Turbulence (const Parameters & parameters) :
    FieldStencil<TurbulentFlowField> ( parameters ), _prefix(parameters.vtk.prefix), _written(false) {}


void VTKStencil4Turbulence::apply ( TurbulentFlowField & flowField, int i, int j ) {

    assertion ( FieldStencil<TurbulentFlowField>::_parameters.geometry.dim == 2 );
    ...
    if ((flowField.getFlags().getValue(i,j) & OBSTACLE_SELF) == 0){
        ...
        viscosityStream << flowField.getViscosityField().getScalar(i,j) << std::endl;
        ...
        viscosityStream << "0.0" << std::endl;
        velocityStream << "0.0 0.0 0.0" << std::endl;
    }
}


void VTKStencil4Turbulence::apply ( TurbulentFlowField & flowField, int i, int j, int k ) {

    assertion ( FieldStencil<TurbulentFlowField>::_parameters.geometry.dim == 3 );
    ...
    if ((flowField.getFlags().getValue(i,j,k) & OBSTACLE_SELF) == 0){
        ...
        pressureStream << pressure << std::endl;
        viscosityStream << flowField.getViscosityField().getScalar(i,j,k) << std::endl;
...
void VTKStencil4Turbulence::openFile ( const TurbulentFlowField & flowField, int timeStep ){
...
void VTKStencil4Turbulence::write ( TurbulentFlowField & flowField, int timeStep ){
    openFile(flowField,timeStep);

    if (FieldStencil<TurbulentFlowField>::_parameters.geometry.dim == 2){
        // Write pressure
        ...
        // Write viscosity
        _ofile << "SCALARS viscosity float 1" << std::endl << "LOOKUP_TABLE default" << std::endl;
        _ofile << viscosityStream.str() << std::endl ;
        viscosityStream.str("");
    ...
    if (FieldStencil<TurbulentFlowField>::_parameters.geometry.dim == 3){
        // Write pressure
...
void VTKStencil4Turbulence::closeFile () {
    _ofile. close ();
}
