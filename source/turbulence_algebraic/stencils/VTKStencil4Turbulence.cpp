...
VTKStencil4Turbulence(const Parameters& parameters)
  : FieldStencil<TurbulentFlowField>(parameters), _prefix(
      parameters.vtk.prefix), _written(false) {}
...
apply(TurbulentFlowField& flowField, int i, int j) {
  ...
  if ((flowField.getFlags().getValue(i, j) & OBSTACLE_SELF) == 0) {
    ...
    viscosityStream << flowField.getViscosityField().getScalar(i, j) <<
      std::endl;
...
apply(TurbulentFlowField& flowField, int i, int j, int k) {
  ...
  if ((flowField.getFlags().getValue(i, j, k) & OBSTACLE_SELF) == 0) {
    ...
    pressureStream << pressure << std::endl;
    viscosityStream << flowField.getViscosityField().getScalar(i, j, k) <<
      std::endl;
...
write(TurbulentFlowField& flowField, int timeStep) {
  ...
  if (FieldStencil<TurbulentFlowField>::_parameters.geometry.dim == 2) {
    ...
    // Write viscosity
    _ofile << "SCALARS viscosity float 1" << std::endl <<
      "LOOKUP_TABLE default" << std::endl;
