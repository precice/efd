...
TurbulencePrandtlViscosityStencil::
TurbulencePrandtlViscosityStencil(
  Parameters& parameters)
  : ComputeLocalViscosityStencil(parameters), _kappa(parameters.blm.kappa),
    _delta99(
      parameters.blm.delta99) {
  std::cout << "Turbulence model with kappa and delta 99 being\t" << _kappa <<
    '\t' << _delta99 << std::endl;
}
...
void
TurbulencePrandtlViscosityStencil::
apply(TurbulentFlowField& flowField,
      int i, int j) {
  ...
  const FLOAT mixingLength = fmin(flowField.getDistanceToWallField().getScalar(
                                    i, j) * _kappa, 0.09 * _delta99);

  // compute turbulent viscosity
  const FLOAT buffer = mixingLength * mixingLength * sqrt(2.0 *
                                                          StressTensorSum2D(
                                                            _localVelocity,
                                                            _localMeshsize));
