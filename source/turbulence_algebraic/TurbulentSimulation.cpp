...
// initialise the Simulation-object as usual AND initialise the reference to the turbulent flow field data
// as well as the new stencils and iterators
TurbulentSimulation::TurbulentSimulation(Parameters &parameters,
TurbulentFlowField &flowField) :
Simulation(parameters, flowField), _stencilFactory(parameters), _turbulentFlowField(flowField),
_computeLocalViscosityStencil(_stencilFactory.getLocalViscosityStencil()),
_computeLocalViscosityIterator(flowField, parameters,*_computeLocalViscosityStencil),
...
void TurbulentSimulation::initializeTurbulenceFlowfield() {
  ...
  if (_parameters.geometry.dim == 2) {
	...
	if ( (_turbulentFlowField.getFlags().getValue(i,j)&OBSTACLE_SELF)==0){
          ...
          // set distance
          _turbulentFlowField.getDistanceToWallField().getScalar(i, j) = distance;
    ...
    for (int k = 2; k <= _parameters.parallel.localSize[2] + 1; k++) {
      for (int j = 2; j <= _parameters.parallel.localSize[1] + 1; j++) {
        for (int i = 2; i <= _parameters.parallel.localSize[0] + 1; i++) {
          ...
          // if this is a fluid cell...
          if ( (_turbulentFlowField.getFlags().getValue(i,j,k)&OBSTACLE_SELF)==0){
            // determine offset from step
            FLOAT stepOffset = 0.0;
            ...
            // set distance
            _turbulentFlowField.getDistanceToWallField().getScalar(i, j, k) = fmin(distanceY,distanceZ);
            // for obstacle cells, set distance to zero
          } else {
            _turbulentFlowField.getDistanceToWallField().getScalar(i, j, k) = 0.0;
          }
        }
      }
    }
...
//	_turbulentFlowField.getPressure().show("pressure");
	...
	// communicate pressure -> using _parallelManager should also work in this case
	_turbulentParallelManager.communicatePressure();

	// compute velocity
	_velocityIterator.iterate();

	// communicate velocity -> using _parallelManager should also work in this case
