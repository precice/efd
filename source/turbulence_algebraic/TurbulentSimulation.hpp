...
/** inherits from the Simulation class and performs one time step, incorporating
 * the
...
private:
  ...
  TurbulentFlowField&        _turbulentFlowField;        // ! New data structure
                                                         // for turbulence
                                                         // simulation
  ComputeLocalViscosityStencil* _computeLocalViscosityStencil; // ! stencil
                                                               // for local
                                                               // viscosity
                                                               // evaluations
  FieldIterator<TurbulentFlowField> _computeLocalViscosityIterator; // !
                                                                    // iterator
                                                                    // for the
                                                                    // local
                                                                    // viscosity
                                                                    // evaluation
                                                                    // stencil
  MaximumTurbulentViscosityStencil _maxTurbViscStencil; // ! stencil for
                                                        // computation of
                                                        // minimal turb.
                                                        // viscosity
  FieldIterator<TurbulentFlowField> _maxTurbViscIterator; // ! iterator for the
                                                          // stencil
  ...
  /** initialises the fields for turbulence simulation (viscosity, distance from
   * wall) */
  void
...
public:
  ...
  plotVTK(int timeStep) {
    ...
    FieldIterator<TurbulentFlowField> vtkIterator(_turbulentFlowField,
                                                  _parameters, vtkStencil, 1,
                                                  0);
...
protected:
  ...
  setTimeStep() {
    ...
    localMin = std::min(_parameters.timestep.dt,
                        std::min(std::min(1 /
                                          (_maxTurbViscStencil.
                                           getMaximumTurbulentViscosity() + 1 /
                                           _parameters.flow.Re) / (2 * factor),
