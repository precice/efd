...
public:
   ...
   */
  Iterator(FlowField&        flowfield,
           const Parameters& parameters) : _flowField(flowfield),
...
private:
  ...
  // Added since the ability to select the iteration domain provides more
  // flexibility
  // @{
...
public:
  FieldIterator(FlowField& flowField, const Parameters& parameters,
                FieldStencil<FlowField>& stencil,
                int lowOffset = 0, int highOffset = 0);

  /**
   * Volume iteration over the field.
   *
   * Volume iteration. The stencil will be applied to all cells in the domain
   * plus the upper
...
private:
  ...
  // This iterator has a reference to a stencil for each side, and will call its
  // methods
  BoundaryStencil<FlowField>& _leftWallStencil; // ! Stencil used on
                                                // the left wall
  BoundaryStencil<FlowField>& _rightWallStencil; // ! Stencil used on
                                                 // the right wall
  BoundaryStencil<FlowField>& _bottomWallStencil; // ! Stencil used on
                                                  // the bottom wall
  BoundaryStencil<FlowField>& _topWallStencil; // ! Stencil used on
                                               // the top wall
  BoundaryStencil<FlowField>& _frontWallStencil; // ! Stencil used on
                                                 // the front wall
  BoundaryStencil<FlowField>& _backWallStencil; // ! Stencil used on
                                                // the back wall
