    ...
    for (j = limitsY[0]; j < limitsY[1]; j++) {
      for (i = limitsX[0]; i < limitsX[1]; i++) {
        ...
        if ((obstacle & OBSTACLE_SELF) == 0) { // If the cell is fluid
          ...
          const FLOAT dy_0 = parameters.meshsize->getDy(cellIndexX, cellIndexY,
                                                        cellIndexZ);
          const FLOAT dy_M1 = parameters.meshsize->getDy(cellIndexX,
                                                         cellIndexY - 1,
                                                         cellIndexZ);
          const FLOAT dy_P1 = parameters.meshsize->getDy(cellIndexX,
                                                         cellIndexY + 1,
                                                         cellIndexZ);
          const FLOAT dz_0 = parameters.meshsize->getDz(cellIndexX, cellIndexY,
                                                        cellIndexZ);
