...
PetscErrorCode
computeMatrix2D(KSP ksp, Mat A, Mat pc, void* ctx);

PetscErrorCode
computeMatrix3D(KSP ksp, Mat A, Mat pc, void* ctx);
#else
PetscErrorCode
computeMatrix2D(KSP ksp, Mat A, Mat pc, MatStructure* matStructure,
                void* ctx);

PetscErrorCode
computeMatrix3D(KSP ksp, Mat A, Mat pc, MatStructure* matStructure,
                void* ctx);

#endif

PetscErrorCode
computeRHS2D(KSP ksp, Vec b, void* ctx);
PetscErrorCode
computeRHS3D(KSP ksp, Vec b, void* ctx);
...
computeMatrix2D(KSP ksp, Mat A, Mat pc, MatStructure* matStructure,
  ...
  for (j = limitsY[0]; j < limitsY[1]; j++) {
    for (i = limitsX[0]; i < limitsX[1]; i++) {
      ...
      if ((obstacle & OBSTACLE_SELF) == 0) { // If we have a fluid cell
        ...
        const FLOAT dx_M1 = parameters.meshsize->getDx(cellIndexX - 1,
                                                       cellIndexY);
        const FLOAT dx_P1 = parameters.meshsize->getDx(cellIndexX + 1,
                                                       cellIndexY);
        const FLOAT dy_0  = parameters.meshsize->getDy(cellIndexX, cellIndexY);
        const FLOAT dy_M1 = parameters.meshsize->getDy(cellIndexX, cellIndexY -
                                                       1);
        const FLOAT dy_P1 = parameters.meshsize->getDy(cellIndexX, cellIndexY +
                                                       1);
      ...
      //        stencilValues[4]);
        ...
        MatSetValuesStencil(A, 1, &row, 5, column, stencilValues,
                 ...
                 OBSTACLE_RIGHT +
                 OBSTACLE_TOP +
                 OBSTACLE_BOTTOM) { // Not fluid, but fluid
        // somewhere around
        int counter = 0; // This will contain how many neighbours are
                         // fluid

        if ((obstacle & OBSTACLE_LEFT) == 0) { // If there is fluid to
                                               // the left
          stencilValues[counter] = 1.0;
          column[counter].i      = i - 1; column[counter].j = j;
          counter++; // We have just identified a fuid cell and
                     // prepared to average
        }
        ...
        MatSetValuesStencil(A, 1, &row, counter + 1, column, stencilValues,
                            INSERT_VALUES);
      } else { // The remaining possibility is that the cell is
               // obstacle surrounded
        ...
        MatSetValuesStencil(A, 1, &row, 1, column, stencilValues,
                            INSERT_VALUES);
      }
    }
  }
  ...
  if (context->setAsBoundary & LEFT_WALL_BIT) {
    for (j = limitsY[0]; j < limitsY[1]; j++) {
      ...
      if (parameters.walls.typeLeft == DIRICHLET) { // If Dirichlet
                                                    // velocity boundary
                                                    // conditions
        // therefore, Neumann in the pressure
        stencilValues[0] = 1;
        stencilValues[1] = -1;
      } else if (parameters.walls.typeLeft == NEUMANN) { // Neumann
                                                         // velocity
                                                         // boundary
                                                         // conditions,
        stencilValues[0] = 0.5;
...
#else
PetscErrorCode
computeMatrix3D(KSP ksp, Mat A, Mat pc, MatStructure* matStructure, void* ctx) {
#endif
  PetscUserCtx* context    = (PetscUserCtx*)ctx;
  Parameters&   parameters = context->getParameters();
  ...
  for (k = limitsZ[0]; k < limitsZ[1]; k++) {
    for (j = limitsY[0]; j < limitsY[1]; j++) {
      for (i = limitsX[0]; i < limitsX[1]; i++) {
        ...
        int const cellIndexX = i - limitsX[0] + 2;
        int const cellIndexY = j - limitsY[0] + 2;
        int const cellIndexZ = k - limitsZ[0] + 2;
        int const obstacle   = flags.getValue(cellIndexX, cellIndexY,
                                              cellIndexZ);

        if ((obstacle & OBSTACLE_SELF) == 0) { // If the cell is fluid
          FLOAT const dx_0 = parameters.meshsize->getDx(cellIndexX,
                                                        cellIndexY,
                                                        cellIndexZ);
          FLOAT const dx_M1 = parameters.meshsize->getDx(cellIndexX - 1,
                                                         cellIndexY,
                                                         cellIndexZ);
          FLOAT const dx_P1 = parameters.meshsize->getDx(cellIndexX + 1,
                                                         cellIndexY,
                                                         cellIndexZ);
          FLOAT const dy_0 = parameters.meshsize->getDy(cellIndexX,
                                                        cellIndexY,
                                                        cellIndexZ);
          FLOAT const dy_M1 = parameters.meshsize->getDy(cellIndexX,
                                                         cellIndexY - 1,
                                                         cellIndexZ);
          FLOAT const dy_P1 = parameters.meshsize->getDy(cellIndexX,
                                                         cellIndexY + 1,
                                                         cellIndexZ);
          FLOAT const dz_0 = parameters.meshsize->getDz(cellIndexX,
                                                        cellIndexY,
                                                        cellIndexZ);
          FLOAT const dz_M1 = parameters.meshsize->getDz(cellIndexX,
                                                         cellIndexY,
                                                         cellIndexZ - 1);
          FLOAT const dz_P1 = parameters.meshsize->getDz(cellIndexX,
                                                         cellIndexY,
                                                         cellIndexZ + 1);

          FLOAT const dx_L  = 0.5 * (dx_0 + dx_M1);
          FLOAT const dx_R  = 0.5 * (dx_0 + dx_P1);
          FLOAT const dx_Bo = 0.5 * (dy_0 + dy_M1);
          FLOAT const dx_T  = 0.5 * (dy_0 + dy_P1);
          FLOAT const dx_F  = 0.5 * (dz_0 + dz_M1);
          FLOAT const dx_B  = 0.5 * (dz_0 + dz_P1);
          ...
          stencilValues[6] = -2.0 / (dx_R * dx_L) - 2.0 / (dx_T * dx_Bo) - 2.0 /
                             (dx_F * dx_B); //
                                            // center
          ...
          MatSetValuesStencil(A, 1, &row, 7, column, stencilValues,
                              INSERT_VALUES);
        } else if (obstacle != 127) { // If non-fluid and still not
                                      // completely surounded
          int counter = 0;

          if ((obstacle & OBSTACLE_LEFT) == 0) { // If there's fluid
                                                 // to the left
          ...
          stencilValues[counter] = (double)(-counter);
          column[counter].i      = i; column[counter].j = j, column[counter].k =
            k;
  ...
  if (context->setAsBoundary & RIGHT_WALL_BIT) {
    for (j = limitsY[0]; j < limitsY[1]; j++) {
      for (k = limitsZ[0]; k < limitsZ[1]; k++) {
        column[0].i = Nx - 1;                     column[0].j = j; column[0].k =
          k;
        column[1].i = context->displacement[1]; column[1].j = j; column[1].k =
          k;
  ...
  if (context->setAsBoundary & BOTTOM_WALL_BIT) {
    for (i = limitsX[0]; i < limitsX[1]; i++) {
      for (k = limitsZ[0]; k < limitsZ[1]; k++) {
        column[0].i = i; column[0].j = 0;                        column[0].k =
          k;
        column[1].i = i; column[1].j = context->displacement[2]; column[1].k =
          k;
  ...
  if (context->setAsBoundary & TOP_WALL_BIT) {
    for (i = limitsX[0]; i < limitsX[1]; i++) {
      for (k = limitsZ[0]; k < limitsZ[1]; k++) {
        column[0].i = i; column[0].j = Ny - 1;                     column[0].k =
          k;
        column[1].i = i; column[1].j = context->displacement[3]; column[1].k =
          k;
  ...
  if (context->setAsBoundary & FRONT_WALL_BIT) {
    for (i = limitsX[0]; i < limitsX[1]; i++) {
      for (j = limitsY[0]; j < limitsY[1]; j++) {
        ...
        column[1].i = i; column[1].j = j; column[1].k =
...
computeRHS2D(KSP ksp, Vec b, void* ctx) {
  ...
  for (j = limitsY[0]; j < limitsY[1]; j++) {
    for (i = limitsX[0]; i < limitsX[1]; i++) {
      const int obstacle = flags.getValue(i - limitsX[0] + 2, j - limitsY[0] +
                                          2);
...
computeRHS3D(KSP ksp, Vec b, void* ctx) {
  ...
  for (k = limitsZ[0]; k < limitsZ[1]; k++) {
    for (j = limitsY[0]; j < limitsY[1]; j++) {
      for (i = limitsX[0]; i < limitsX[1]; i++) {
        int const obstacle = flags.getValue(i - limitsX[0] + 2,
                                            j - limitsY[0] + 2,
                                            k - limitsZ[0] + 2);

        if ((obstacle & OBSTACLE_SELF) == 0) { // If this is a fluid
                                               // cell
          array[k][j][i] = RHS.getScalar(i - limitsX[0] + 2,
                                         j - limitsY[0] + 2,
                                         k - limitsZ[0] + 2);
        } else {
          array[k][j][i] = 0.0;
...
// ignore them.
void
createLimits(Parameters& parameters,
             DM&         da,
             int*        limitsX,
             int*        limitsY,
             int*        limitsZ) {
  ...
  // Check right wall
  if (parameters.parallel.indices[0] == parameters.parallel.numProcessors[0] -
      1) {
    limitsX[1]--;
  }
  ...
  // Check top wall
  if (parameters.parallel.indices[1] == parameters.parallel.numProcessors[1] -
      1) {
    limitsY[1]--;
  }
  ...
  // Check back wall
  if (parameters.parallel.indices[2] == parameters.parallel.numProcessors[2] -
      1) {
    limitsZ[1]--;
  }
}
