...
solve() {
  ...
  int nx = _flowField.getNx(), ny = _flowField.getNy(), nz =
    _flowField.getNz();
      ...
      for (int k = 2; k < nz + 2; k++) {
        for (int j = 2; j < ny + 2; j++) {
          for (int i = 2; i < nx + 2; i++) {
            ...
            const FLOAT a_C = -2.0 / (dx_E * dx_W) - 2.0 / (dx_N * dx_S) - 2.0 /
                              (dx_B * dx_T);
      ...
      for (int k = 2; k < nz + 2; k++) {
        for (int j = 2; j < ny + 2; j++) {
          for (int i = 2; i < nx + 2; i++) {
            ...
            resnorm += pow((
                             _flowField.getRHS().getScalar(i, j, k)
                             - a_W * P.getScalar(i - 1, j, k) - a_E *
                             P.getScalar(i + 1, j, k)
                             - a_S * P.getScalar(i, j - 1, k) - a_N *
                             P.getScalar(i, j + 1, k)
                             - a_B * P.getScalar(i, j, k - 1) - a_T *
                             P.getScalar(i, j, k + 1)
    ...
    do {
      for (int j = 2; j < ny + 2; j++) {
        for (int i = 2; i < nx + 2; i++) {
          ...
          const FLOAT gaussSeidel = 1.0 / a_C * (_flowField.getRHS().getScalar(
                                                   i, j)
                                                 - a_W * P.getScalar(i - 1, j) -
                                                 a_E * P.getScalar(i + 1, j)
                                                 - a_S * P.getScalar(i, j - 1) -
                                                 a_N * P.getScalar(i, j + 1));
          P.getScalar(i, j) = omg * gaussSeidel + (1.0 - omg) * P.getScalar(i,
                                                                            j);
        }
      }
      ...
      for (int j = 2; j < ny + 2; j++) {
        for (int i = 2; i < nx + 2; i++) {
          ...
          const FLOAT residual = _flowField.getRHS().getScalar(i, j)
                                 - a_W * P.getScalar(i - 1, j) - a_E *
                                 P.getScalar(i + 1, j)
                                 - a_S * P.getScalar(i, j - 1) - a_N *
                                 P.getScalar(i, j + 1)
                                 - a_C * P.getScalar(i, j);
